...
```{r, eval = FALSE}
#library(devtools)
#install_github("Programming-The-Next-Step-2022/EatConscious")
```
...
The water footprint functions were created using data from the Water Footprint Network. The raw dataset can be found in the GitHub repository.

#### Measures

The Water Footprint of each food item is measured as the cubic Liters of water consumed per ton of the food item produced (cubicL/ton).
...
You should input the country name according to the FIPS country codes.

```{r}
# You can use the countrycodes() function to check what country code corresponds to which country within a specified continent.  
